from bsg_ascii_to_rom import bsg_ascii_to_rom

RC = [
    (2**i if i < 8 else (2**i) ^ (0x11B << (i - 8)))
    for i in range(14)
]

SBOX = [
    
]

def round_key(k, r=0):
    if r == 0:
        return k
    kl = len(k)

    kg = [SBOX[v] for v in k[-3:] + [k[kl-4]]]
    print("kg: ", end="")
    print(''.join([f'{v:02x}' for v in kg]))

    if r - 1 < 8:
        kg[0] ^= 2**(r-1)
    else:
        kg[0] ^= 2**(r-1) ^ (0x11B << (r- 9))
        
    print(f'kg0: {kg[0]:08x}')
            
    for i in range(kl):
        if i < 4:
            k[i] ^= kg[i]
        elif i // 4 == 4:
            k[i] ^= SBOX[k[i-4]]
        else:
            k[i] ^= k[i-4]
    return k

def all_key(k):
    kr = 7 if len(k) == 32 else 8 if len(k) == 24 else 10
    kz = 240 if len(k) == 32 else 208 if len(k) == 24 else 176

    k_all = [k]
    for i in range(1, kr+1):
        knew = round_key(k_all[i-1][:], i)
        # print(''.join([f'{v:02x}' for v in knew]))
        k_all.append(knew)

    return [item for sublist in k_all for item in sublist][:kz]

def gen_sbox():
    P = int('100011011', 2)  # x^8 + x^4 + x^3 + x + 1
    Q = 0x63

    # Create template box
    t = [0] * 256
    t[0] = 1
    for i in range(1, 256):
        t[i] = t[i-1]
        if t[i] & 0x80:  # if the high bit is set
            t[i] = ((t[i] << 1) ^ P) & 0xFF  # XOR with P after shifting left
        else:
            t[i] = (t[i] << 1) & 0xFF

    # Create S-box using t
    box = [0] * 256
    box[0] = Q
    for i in range(1, 256):
        x = t[255 - i]
        x |= x << 8
        x ^= Q ^ (x >> 4) ^ (x >> 5) ^ (x >> 6) ^ (x >> 7)
        box[t[i]] = x & 0xFF

    return box

if __name__ == '__main__':
    # print(', '.join([f'{v:02x}' for v in gen_sbox()]))
    # k0 = [100]*32
    # print("Key0:", ''.join([f'{v:02x}' for v in k0]))

    # kr = all_key(k0)
    # kl = ',\n'.join([''.join([f'{v:02x}' for v in kr[i:i+16]]) for i in range(0, len(kr), 16)])

    # print("KeyList:", len(kr) // 16)
    # print(kl)

    # k_all = [k0]
    # i = 1
    # knew = round_key(k_all[i-1][:], i)
    # print(''.join([f'{v:02x}' for v in knew]))


    # for i in range(len(SBOX)):
    #     #print(f"8'h{format(i, '02x')}: rom_data = 8'h{format(SBOX[i], '02x')};")

    #     print(f"#10 rom_addr = 8'h{format(i, '02x')};")
    #   #10 rom_addr = 8'h01;
    
    f = open("rom_sbox.txt", "w")
    
    cnt = 0

    for i in SBOX:
        #  print lines of verilog binary strings, e.g.
        #    1001_10101_10011_1101
        # f.write(f"{i:008b}\n")
        
        f.write(f"8'h{cnt:02x}: data_o = 8'h{i:02x};\n")
        cnt += 1
        
        # print(f"{i:008b}")
    f.close()
        
    # bsg_ascii_to_rom("rom_sbox.txt", "rom_sbox", zero=1, spool=open("rom_sbox.v", "w"))
