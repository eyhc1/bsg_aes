...
def all_key(k):
    ...
    return [item for sublist in k_all for item in sublist][:240]

def key_expansion(key : str):
    k = bytes.fromhex(key)
    hex_list = [int.from_bytes(k[i:i+1], byteorder='big') for i in range(len(k))]
    kr = all_key(hex_list)
    return ''.join([f'{v:02x}' for v in kr])